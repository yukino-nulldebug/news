...
| Providerモックテスト | RSS、NewsAPIスタブ、Alpha Vantage、HTTP共通処理を外部通信なしで検証する |
...
| NewsAPI | Phase 4 の未対応スタブとして、通信せず警告を返すことを確認する |
...
既存の `sample_data/` と `sample_data_edge/` をテストで利用する。
ただし、テスト中に内容を変更しない。
...
2. `tests/conftest.py` を追加し、Settings、ReportData、固定RSS/JSON、疑似HTTPレスポンスのfixtureを定義する。
...
6. `tests/unit/test_news_fetcher.py` と `tests/unit/test_market_fetcher.py` を追加し、sample JSON検証を自動化する。
7. `tests/unit/test_settings.py` を追加し、環境変数と設定値のパースを検証する。
8. `tests/unit/test_http_client.py` を追加し、HTTP共通処理とURLマスクを検証する。
9. `tests/providers/test_rss_provider.py` を追加し、RSS正規化と警告を検証する。
10. `tests/providers/test_alpha_vantage_provider.py` を追加し、マーケットAPIレスポンス正規化とAPIキー未設定警告を検証する。
11. `tests/providers/test_newsapi_provider.py` を追加し、未対応スタブの動作を検証する。
12. `tests/integration/test_sample_mode.py` を追加し、sampleモードのレポート生成を検証する。
13. `tests/integration/test_api_mode.py` を追加し、モック外部データによるapiモードを検証する。
14. `tests/security/test_secret_masking.py` を追加し、機密情報が出力されないことを検証する。
15. `python3 -m pytest` を実行し、全テストが成功することを確認する。
16. `APP_MODE=sample python3 main.py` を実行し、既存の手動実行が壊れていないことを確認する。
