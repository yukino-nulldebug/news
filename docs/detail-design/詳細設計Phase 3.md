...
そのため、Phase 3 では `previous_close` を実質必須項目として扱う。

| 条件 | Phase 3 の扱い |
| --- | --- |
| `previous_close` が欠損 | `MarketCalculationError` とし、終了コード `1` で停止する |
| `previous_close` が非数値 | `MarketCalculationError` とし、終了コード `1` で停止する |
| `previous_close == 0` | 計算不可として `change=None`、`change_rate=None` を設定し、`WARNING F-05` を出して継続する |
| `previous_close` が数値かつ0以外 | 前日比・変化率を計算する |

これにより、基本設計上の任意項目と、Phase 3 の計算処理上の必須条件を分けて扱う。
...
| `previous_close` が `None` | `MarketCalculationError` が発生する |
...
- Phase 3 では `previous_close` が実質必須として扱われ、欠損時は `MarketCalculationError` になる。
