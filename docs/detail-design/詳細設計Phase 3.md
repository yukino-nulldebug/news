...
| `src/utils/execution_result.py` | 変更なしまたは確認 | 警告件数、マーケットコメント件数を集計する |
...
`max_length` が `1` から `3` の場合は、`...` 自体が上限を超えるため、上限文字数分のピリオドで表す。
...
8. `max_length <= 3` の場合は `.` を `max_length` 個返す。
9. それ以外は `normalized[: max_length - 3].rstrip() + "..."` を返す。
...
`format_summary()` は代替文を返すだけにし、ログ出力は `main.py` で行う。
...
Phase 3 では、`sample_data/market.json` から読み込んだ日経平均、S&P500、USD/JPY を最低対象とする。
...
| 指標 | 現在値 | 前日終値 | 前日比 | 変化率 |
| --- | ---: | ---: | ---: | ---: |
| 日経平均 | `38500.25` | `38200.00` | `300.25` | `0.79` |
| S&P500 | `5250.10` | `5200.00` | `50.10` | `0.96` |
...
4. 必須キーがない場合は `MarketCalculationError` を送出する。
5. `calculate_change()` を呼び出す。
6. 戻り値を `change` と `change_rate` に設定する。
7. 計算済みリストへ追加する。
...
| `current_value` が非数値 | `MarketCalculationError` | `ERROR F-05` | 保存しない | `1` |
| `previous_close` が非数値 | `MarketCalculationError` | `ERROR F-05` | 保存しない | `1` |
| 必須キー欠損 | `MarketCalculationError` | `ERROR F-05` | 保存しない | `1` |

`previous_close == 0` は、データとしては不完全だがレポート全体を止めるほどではないため、警告として処理を継続する。
その他の型不正や必須キー欠損は、計算処理の前提を満たさないため停止する。
...
| `current_value` が非数値 | `MarketCalculationError` | 停止 | `ERROR F-05` | `1` |
| `previous_close` が非数値 | `MarketCalculationError` | 停止 | `ERROR F-05` | `1` |
| `current_value` 欠損 | `MarketCalculationError` | 停止 | `ERROR F-05` | `1` |
| `previous_close` 欠損 | `MarketCalculationError` | 停止 | `ERROR F-05` | `1` |
...
```mermaid
flowchart TD
    A([整形または計算で継続不可エラー]) --> B{例外種別}
    B -->|SummaryFormatError| C[ERROR F-03]
    B -->|MarketCalculationError| D[ERROR F-05]
...
1. 国内ニュースを読み込む。
2. 海外ニュースを読み込む。
3. `format_news_items()` で概要文を整形する。
4. 概要文欠損があれば `record_recoverable_warning()` を呼ぶ。
5. マーケット情報を読み込む。
6. `calculate_market_changes()` で前日比・変化率を計算する。
7. `change_rate is None` の指標があれば `record_recoverable_warning()` を呼ぶ。
8. `generate_market_comments()` を呼び、コメント件数を集計する。
9. `ReportData` を返す。
...
| `format_news_items(items, max_length)` | `list[dict]`, `int` | `list[dict]` | 各ニュースへ `short_summary` を追加 |
...
| `generate_market_comments(items)` | 変化率に応じた中立的コメントを生成する |
| `generate_report(report_data)` | 整形済みニュース、計算済みマーケット、警告を含むMarkdownを生成する |
...
| `current_value` が文字列 | `MarketCalculationError` が発生する |
| `previous_close` が文字列 | `MarketCalculationError` が発生する |
| `current_value` が `True` | `MarketCalculationError` が発生する |
| `previous_close` が欠損 | `MarketCalculationError` が発生する |
...
4. `main.py` で概要文欠損を検出し、`WARNING F-03` として記録する。
...
7. `main.py` で `previous_close == 0` による計算不可を `WARNING F-05` として記録する。
8. `src/report/generator.py` で `short_summary`、前日比、変化率、`N/A` を表示する。
9. `generate_market_comments()` の禁止表現を確認する。
10. `python main.py` を実行し、`reports/YYYY-MM-DD.md` と `logs/app.log` を確認する。
...
- `SUMMARY_MAX_LENGTH <= 0` またはマーケット数値不正の場合は、`ERROR` ログを出して終了コード `1` になる。
