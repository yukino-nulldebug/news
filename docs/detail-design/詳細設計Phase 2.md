...
| `.env` によるAPIキー読み込み | Phase 4 で実装する。ただし Phase 2 では既存の設定読み込み処理を利用し、`LOG_DIR`、`REPORT_DIR`、`SUMMARY_MAX_LENGTH` などの基本設定値は扱う |
...
`setup_logger()` はファイル出力の初期化失敗を外へ投げず、標準出力ハンドラーだけを持つロガーを返す。
このため、ログ初期化失敗はアプリ全体の停止理由にしない。
...
| `LoggingSetupError` | `logger.py` | `F-09` | 原則送出しない。ファイル出力に失敗した場合は標準出力のみで継続 |
...
| `setup_logger(log_dir)` | `Path` | `logging.Logger` | 標準出力と `logs/app.log` へ出力するロガーを作成する。ファイル出力に失敗した場合も例外を外へ投げず、標準出力ロガーを返す |
...
| `LoggingSetupError` | ログファイル設定失敗を表す例外。ただし Phase 2 の実装では `setup_logger()` が標準出力へフォールバックし、原則として外へ送出しない |
...
| `load_news_items(file_path, feature_id)` | 機能IDを受け取り、指定されたJSONファイルを読み込む。国内/海外どちらの失敗かを `feature_id` で識別する |
| `fetch_sample_news(settings)` | Phase 2 では必須にしない。実装する場合も内部で `load_news_items(settings.news_jp_path, "F-01")` と `load_news_items(settings.news_global_path, "F-02")` を呼ぶ薄いラッパーに限定する |

Phase 2 の基本呼び出しは、`main.py` から以下のように明示的に行う。

```python
domestic_news = load_news_items(settings.news_jp_path, "F-01")
global_news = load_news_items(settings.news_global_path, "F-02")
```

これにより、国内ニュース取得と海外ニュース取得のログ、例外、終了コード判定を機能ID単位で分けられる。
...
Phase 2 の `ReportData` は、Phase 1 のキー名と同じ `news_domestic`、`news_global`、`markets` に統一する。
`jp_news` / `global_news` のような別名は使わず、ニュースJSON内の `region` 値である `domestic` / `global` と対応させる。
