...
| 対象 | 内容 |
| --- | --- |
| README整備 | 概要、機能、セットアップ、sample実行、api実行、テスト、出力例、注意事項を記載する |
...
- `api` モードではRSSニュースとマーケットAPIから外部データを取得できる。
...
| ネットワーク | sampleモードと単体テストでは不要。apiモードでは必要 |
...
### 3.10 `Alpha Vantageでマーケット情報を取得する`
...
2. READMEを日本語中心の構成に更新し、概要、機能、セットアップ、sample実行、api実行、テスト、出力、ログ、注意事項を記載する。
...
- READMEに、概要、主な機能、必要環境、セットアップ、sample実行、api実行、テスト、出力、ログ、環境変数、注意事項が記載されている。
