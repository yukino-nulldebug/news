...
- 取得したニュース概要文を整形する。
- 日本市場参考指標、米国市場参考指標、USD/JPYの前日比と変化率を計算する。
...
flowchart TD
    ...
    D -->|api| F2[Market API<br>日本市場参考指標 / 米国市場参考指標 / USD-JPY取得]
...
| `src/market/` | 日本市場参考指標、米国市場参考指標、USD/JPYの取得、前日比・変化率計算 | F-04, F-05 |
...
| 対象 | 種別 | 取得項目 |
| --- | --- | --- |
| 日本市場参考指標 | 株価指数または参考ETF | 現在値、前日終値、前日比、変化率 |
| 米国市場参考指標 | 株価指数または参考ETF | 現在値、前日終値、前日比、変化率 |
| USD/JPY | 為替 | 現在値、前日終値、前日比、変化率 |

Alpha Vantage等の無料APIでは、日経平均やS&P500そのものを直接取得できない場合がある。
そのため、外部取得ではProvider側で取得可能な参考ETFやシンボルを `.env` で指定できる構成とする。
...
| 株価・指数・為替 | Alpha Vantage / FMP / marketstack | 日本市場参考指標、米国市場参考指標、USD/JPY等の取得 | 無料枠はリクエスト制限あり。 |
...
| `disclaimer` | string | 投資助言ではない旨の注意書き。 |
| `execution_summary` | dict | 実行ステータス、件数、警告、エラーの集計。 |
...
| F-04 | 日本市場参考指標、米国市場参考指標、USD/JPYのデータを扱える。 |
...
- 再現性: `sample` モードにより、APIキーなしでも第三者がローカルで動作確認できることを示す。
- 外部依存の扱い: RSS/API取得、Provider分岐、timeout、retry、連続取得間隔を設定として扱えることを示す。
- 失敗設計: 欠損値は `N/A` と警告、外部データ全滅は終了コードで分け、同じ成功に見せないことを示す。
- 公開安全性: APIキーを公開しない設計、`.env.example`、`.gitignore`、マスク処理、サンプルデータにより安全性を示す。
- 品質保証: 要件IDと設計項目、テスト観点を対応させ、要件から実装・回帰確認へ落とし込めることを示す。
- AI協働: AIで要件整理、実装案、コードレビューを加速し、最終的な仕様判断、差分確認、テスト追加を自分で行えることを示す。
...
| 価格データ取得元 | 未決 | 日本市場参考指標、米国市場参考指標、USD/JPYを安定取得できるAPIまたは参考ETFを決める。 |
